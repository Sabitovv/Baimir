...
const StrategicCollectionBannerCarousel: FC<StrategicCollectionBannerCarouselProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className='pointer-events-none absolute -right-10 -top-8 h-28 w-28 rounded-full bg-[#238ECE]/20 blur-2xl' />

      <div className='relative sm:hidden'>
        <p className='mb-1 inline-flex rounded-full border border-[#93C9EA]/45 bg-[#93C9EA]/12 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#B9D6EC]'>
        ...
        {periodLabel && (
          <p className='mt-1 inline-flex rounded-full border border-white/20 bg-white/10 px-2 py-0.5 text-[9px] font-semibold text-[#CFE4F3]'>
        ...
        <Link
          ...
          className='mt-2.5 inline-flex rounded-full bg-[#238ECE] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-white transition hover:bg-[#066AAB]'
        >
      ...
      <div className='relative hidden min-h-[150px] items-center gap-4 sm:grid xl:hidden sm:grid-cols-[1fr_1.1fr_auto]'>
        <div className='min-w-0'>
          <p className='mb-1.5 inline-flex rounded-full border border-[#93C9EA]/45 bg-[#93C9EA]/12 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#B9D6EC]'>
            {t('collections.strategic')}
          </p>
          ...
          {periodLabel && (
            <p className='mt-1.5 inline-flex rounded-full border border-white/20 bg-white/10 px-2.5 py-0.5 text-[10px] font-semibold text-[#CFE4F3]'>
              {t('collections.promo')} {periodLabel}
            </p>
          )}
        </div>
        ...
        <div className='min-w-0 text-center'>
          ...
          <Link
            to={`/catalog/product/${coverProduct.slug}`}
            className='mt-2 inline-flex rounded-full bg-[#238ECE] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-white transition hover:bg-[#066AAB] md:mt-2.5 md:px-3.5 md:py-1.5 md:text-xs'
          >
            {t('collections.toProducts')}
          </Link>
        </div>
      ...
      <div className='relative hidden min-h-[172px] xl:block'>
        <div className='absolute left-0 top-1/2 z-20 max-w-[38%] -translate-y-1/2'>
          <p className='mb-2 inline-flex rounded-full border border-[#93C9EA]/45 bg-[#93C9EA]/12 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#B9D6EC] sm:mb-2.5 sm:px-3 sm:text-[11px]'>
            {t('collections.strategic')}
          </p>
          ...
          {periodLabel && (
            <p className='mt-2 inline-flex rounded-full border border-white/20 bg-white/10 px-2.5 py-1 text-[10px] font-semibold tracking-[0.02em] text-[#CFE4F3] sm:mt-2.5 sm:px-3 sm:text-xs'>
              {t('collections.promo')} {periodLabel}
            </p>
          )}
        </div>
        ...
        <div className='absolute bottom-0 left-1/2 z-20 -translate-x-1/2'>
          <Link
            to={`/catalog/product/${coverProduct.slug}`}
            className='inline-flex rounded-full bg-[#238ECE] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-white transition hover:bg-[#066AAB] sm:px-3.5 sm:py-1.5 sm:text-xs'
          >
            {t('collections.toProducts')}
          </Link>
        </div>
