...
// const year = new Date().getFullYear()
  ...
  return (
    <footer className="min-h-[600px] bg-[#171B25] text-white py-12 px-6 md:min-h-[240px] md:px-20 text-center md:text-left">
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-5">
            ...
            <div className="mt-6 flex min-h-10 flex-wrap items-center justify-center gap-3 md:justify-start">
              {social.map((data) => (
                <a
                  ...
                  title={data.platform}
                  className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/30 text-white transition hover:border-[#FF4610] hover:text-[#FF4610]"
                >
          ...
          <div>
            ...
            {phones.length > 0 ? (
              <div className="mt-3 min-h-[3rem] space-y-1 text-sm text-gray-200">
                {phones.map((phone) => (
                  <p key={phone}>
                    <a href={`tel:${normalizePhoneHref(phone)}`} className="hover:text-[#FF4610] transition-colors">
          ...
          <div>
                  ...
                  return (
                    <p key={manager.id || `${managerName}-${managerPhone}`}>
                      ...
                      {managerPhone !== '-' && (
                        <>
                          <br />
                          <a href={`tel:${normalizePhoneHref(managerPhone)}`} className="hover:text-[#FF4610] transition-colors">
                            {managerPhone}
                          </a>
                        </>
                      )}
            ...
            <p className="mt-3 break-all text-sm text-gray-200">
              <a href={`mailto:${contactEmail}`} className="hover:text-[#FF4610] transition-colors">
                {contactEmail}
              </a>
            </p>
          </div>
