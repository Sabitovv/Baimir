    ...
    >
      <div className={`relative flex items-center justify-center overflow-hidden ${isMini ? "h-16 aspect-square mb-1" : isCompact ? "h-24 aspect-[4/3] sm:h-28 md:h-28 lg:h-32 mb-1.5 sm:mb-2 md:mb-2 lg:mb-2.5" : "h-28 aspect-[4/3] sm:h-32 md:h-28 lg:h-32 xl:h-40 mb-2 sm:mb-2.5 md:mb-2 lg:mb-3 xl:mb-4"}`}>
        {isNew === true && !isOutOfStock && (
          <span className={`absolute left-2 top-2 z-20 inline-flex items-center rounded-full bg-[#FFF2EE] text-[#E03A08] font-semibold ${isCompact ? "px-1.5 py-0.5 text-[9px] sm:px-2 sm:text-[10px]" : "px-2 py-0.5 sm:px-2.5 sm:py-1 text-[10px] sm:text-[11px]"}`}>
            {t("commonCatalog.new")}
          </span>
        )}
        {discountPercent !== null && discountPercent > 0 && (
          <span className={`absolute right-2 top-2 z-20 inline-flex items-center rounded-full bg-gradient-to-r from-[#C63A08] to-[#E03A08] text-white font-extrabold shadow-sm ring-1 ring-white/60 ${isCompact ? "px-1.5 py-0.5 text-[9px] sm:px-2 sm:text-[10px]" : "px-2 py-0.5 sm:px-2.5 sm:py-1 text-[10px] sm:text-[11px]"}`}>
            -{discountPercent}%
          </span>
        )}
        <img
      ...
      <div className="mt-auto">
        ...
        <div className="flex gap-2 items-stretch">
          {showCompare && (
            <button
              ...
              className={`flex shrink-0 items-center justify-center rounded-sm border transition ${isCompact ? "h-8 w-8 sm:h-9 sm:w-9" : "h-9 w-9 sm:h-10 sm:w-10"} ${
                isInCompare
                  ? "border-[#FF4610] bg-[#FFF2EE] text-[#E03A08]"
          ...
          {cartItem ? (
            <div className="flex w-full items-center justify-between bg-[#FF4610] rounded-sm">
              <button
                type="button"
                className={`${isMini ? "w-6 h-6 text-xs" : isCompact ? "w-8 h-8 sm:w-9 sm:h-9 text-sm" : "w-9 h-9 sm:w-10 sm:h-10"} flex items-center justify-center text-white font-bold hover:bg-[#E03A08] transition`}
                onClick={(event) => {
                  event.preventDefault();
            ...
            <button
              type="button"
               className={`w-full font-extrabold uppercase rounded-sm transition bg-[#FF4610] text-white hover:bg-[#E03A08] ${isMini ? "py-1 text-[9px]" : isCompact ? "py-1.5 sm:py-2 text-[10px] sm:text-xs md:text-[13px]" : "py-2 text-[11px] sm:text-[13px] md:text-xs lg:text-sm"}`}
              onClick={(event) => {
                event.preventDefault();
