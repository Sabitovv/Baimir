...
const StoragePage = () => {
  ...
  const activeImage = storageImages[activeImageIndex] ?? storageImages[0];
    ...
    <PageContainer>
        ...
        <main className="w-full min-w-0">
          <section>
                ...
                (blogsData?.content?.length ?? 0) > 0 && (
                  <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {blogsData?.content.map((post, index) => {
                      const rawPost = post as unknown as Record<
                        string,
                        unknown
                      >;
                      const title =
                        pickLocalized(post.title, i18n.language) ||
                        t("blogPage.untitled");
                      ...
                      const authorName = pickStringField(rawPost, [
                        "authorName",
                        "author",
                        "author_name",
                      ]);
                      const publishedRaw = pickStringField(rawPost, [
                        "publishedAt",
                        "createdAt",
                        "created_at",
                        "date",
                      ...
                      const readingTime = pickNumberField(rawPost, [
                        "readingTime",
                        "readingMinutes",
                        "reading_time",
                      ]);
                      const viewsCount = pickNumberField(rawPost, [
                        "viewsCount",
                        "viewCount",
                        "views",
                        "views_count",
            ...
            <ScrollReveal className="flex justify-center sm:justify-end mt-8">
              <Link
                to="/blog"
                className="font-bold text-base sm:text-lg text-[#FF4610] flex items-center gap-2 hover:translate-x-1 transition-transform"
              >
                {t("storage.other")}
      ...
      >
        <div className="w-full lg:w-1/2">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-rubik font-semibold mb-6 sm:mb-8 uppercase text-center lg:text-left text-gray-900">
            {t("storage.contact")}
          </h2>
