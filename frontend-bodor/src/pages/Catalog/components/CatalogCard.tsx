  ...
  return (
    ...
    >
      <div className="h-40 flex items-center justify-center mb-4 relative">
        {(product.newProduct ?? product.new) === true &&
          product.inStock !== false && (
            <span className="absolute left-2 top-2 z-20 inline-flex items-center rounded-full bg-[#FFF2EE] px-2.5 py-1 text-[11px] font-semibold text-[#E03A08]">
              {t("commonCatalog.new")}
            </span>
          )}
        {discountPercent !== null && discountPercent > 0 && (
          <span className="absolute right-2 top-2 z-20 inline-flex items-center rounded-full bg-gradient-to-r from-[#C63A08] to-[#E03A08] px-2.5 py-1 text-[11px] font-extrabold text-white shadow-sm ring-1 ring-white/60">
            -{discountPercent}%
          </span>
        )}
        <img
      ...
      <div className="mt-auto">
        ...
        {cartItem ? (
          <div className="flex items-center justify-between bg-[#FF4610] rounded-sm">
            <button
              type="button"
              className="w-10 h-10 flex items-center justify-center text-white font-bold hover:bg-[#E03A08] transition"
              onClick={(event) => {
          ...
          <button
            ...
            className={`w-full py-2 text-sm font-extrabold uppercase rounded-sm transition ${
              product.inStock === false
                ...
                : "bg-[#FF4610] text-white hover:bg-[#E03A08]"
            }`}
